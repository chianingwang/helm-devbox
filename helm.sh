...
helm init
sleep 5s
until [ $(kubectl get pods --namespace kube-system | grep tiller-deploy | grep Running -ci) == 1 ];do echo "Before patch, Tiller-Deploy Containers Not Ready then Sleep 2s .. " && sleep 2s; done
kubectl get pods --namespace kube-system
kubectl create serviceaccount --namespace kube-system tiller
kubectl create clusterrolebinding tiller-cluster-rule --clusterrole=cluster-admin --serviceaccount=kube-system:tiller
kubectl patch deploy --namespace kube-system tiller-deploy -p '{"spec":{"template":{"spec":{"serviceAccount":"tiller"}}}}'
until [ $(kubectl get pods --namespace kube-system | grep tiller-deploy | grep Running -ci) == 1 ];do echo "After patch, Tiller-Deploy Containers Not Ready then Sleep 2s .. " && sleep 2s; done
kubectl get pods --namespace kube-system
helm create mychart
helm install --debug --dry-run ./mychart
echo "Congradulation your helm, k8s and docker dev box is ready !!!"
echo "$ vagrant ssh helm"
